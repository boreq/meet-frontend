...
export default class AppNotifications extends Vue {
    ...
    mounted(): void {
        this.$root.$on(AppNotifications.notificationEvent, (notification: Notification) => {
    ...
    shouldHide(notification: Notification): boolean {
        ...
        return secondsDuration > AppNotifications.visibilityDuration;
    }

    private processErrors(): void {
        this.notifications = this.notifications.filter(notification => {
            const secondsDuration = this.duration(new Date(), notification.created);
            return secondsDuration < (AppNotifications.visibilityDuration + AppNotifications.animationDuration);
        });
    }
